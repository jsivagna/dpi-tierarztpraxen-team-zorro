...
def transformiere_tabelle(con, tabellen_name):
    ...
    # 4. Datums-Bereinigung (YYYY-MM-DD ISO Format)
    ...
    for col in telefon_spalten:
        if col in df.columns:
            ...
            print(f"   -> Spalte '{col}' wurde in E.164-Format (+49...) transformiert.")
...
def erstelle_norm_tabellen(con):
    ...
    # ---------------------------------------------------------
    print("\n   -> Erstelle transform.norm_kunde...")
    con.execute("DROP TABLE IF EXISTS transform.norm_kunde;")

    # WICHTIG: Das 'r' vor den drei Anführungszeichen verhindert SyntaxWarnings in Python
    query_kunde = r"""
